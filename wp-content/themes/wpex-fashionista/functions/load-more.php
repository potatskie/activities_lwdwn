...
function aq_load_more($page = 2, $archive = null) {
	...
	if( $maxpage >= 2) {
		...
		$output .= '<div id="load-more">';
			...
			$output .= '<a href="#" data-pagenum="'. $page .'" data-maxpage="'. $maxpage .'" data-archive_type="'. $archive_type .'" data-archive_id="'. $archive_id .'" data-post_format="'. $post_format .'" data-author="'. $author .'" data-archive_month="'. $archive_month .'" data-archive_year="'. $archive_year .'" data-s="'.get_query_var('s').'" alt="'. __('Load More', 'wpex').'">';
				$output .= __('load more', 'wpex');
			$output .= '</a>';
		$output .= '</div>';
	}
